// Main Invoice Model based on InvoicesDoc-v2.0.1.xsd
...
export interface AadeBookInvoiceType {
   ...
   * @deprecated Commented out in InvoicesDoc-v2.0.1.xsd; superseded by
   * `deliveryLifecycle`. Retained so v1.0.x callers still compile.
   */
  otherTransportDetails?: TransportDetailType[]; // Optional Array (0..unbounded)
  /** Delivery-note lifecycle state. Read-only: set by myDATA on retrieval. */
  invoiceDeliveryStatus?: InvoiceDeliveryStatusType; // Optional, v2.0.1
  /** Delivery lifecycle events. Read-only: provided by myDATA on retrieval. */
  deliveryLifecycle?: DeliveryLifecycle; // Optional, v2.0.1
  /** URL for downloading the invoice (submitted by e-invoicing providers). */
  downloadingInvoiceUrl?: string; // xs:string, Optional, v2.0.1
}
...
export interface InvoiceHeaderType {
  ...
  /** Indicates a reverse movement delivery note (type 9.3) */
  reverseDeliveryNote?: boolean; // xs:boolean, Optional, v2.0.1
  /** Reason the reverse delivery note was issued */
  reverseDeliveryNotePurpose?: ReverseDeliveryNotePurposeType; // Optional, v2.0.1
  /** Indicates the goods are to be weighed */
  toWeigh?: boolean; // xs:boolean, Optional, v2.0.1
}
...
export interface InvoiceRowType {
  ...
  /** Movement purpose for this line, when it differs from the header's */
  movePurposeLine?: number; // xs:int 1..20, Optional, v2.0.1
  /** Title of the other movement reason (when movePurposeLine = 19) */
  otherMovePurposeLineTitle?: string; // xs:string, Optional, MaxLength 150, v2.0.1
...
export interface OutcomeDetailsType {
  /** Whether the goods were delivered in full, in part, or not at all */
...
export interface RejectionDetailsType {
  /** Free-text rejection reason */
  reason?: string; // xs:string, Optional
}

/**
 * One event in a delivery note's lifecycle.
 * Corresponds to <inv:DeliveryEventType> (TransportTypes-v2.0.1.xsd).
 */
export interface DeliveryEventType {
  /** e.g. 'RegisterTransfer', 'ConfirmOutcome', 'Rejection' */
  eventType: string; // xs:string
  /** When the event occurred */
  eventTimestamp: string; // xs:dateTime
  /** VAT number of the user who created the event */
  actorVat: string; // xs:string
  /** Unique registration number of the event, generated by myDATA */
  mark?: number; // xs:long, Optional
  /** Present on movement-start and transhipment events */
  transportDetails?: TransportDetailType;
  /** Present on delivery-outcome events */
  outcomeDetails?: OutcomeDetailsType;
  /** Present on rejection events */
  rejectionDetails?: RejectionDetailsType;
}

/** Wrapper for the delivery lifecycle event list. */
export interface DeliveryLifecycle {
  deliveryEvents: DeliveryEventType[];
}
...
export interface ProviderSignatureType {
  ...
  /** End-to-end payment reference (IRIS via the DIAS interbank system) */
  EndToEndReferenceID?: string; // xs:string, Optional, v2.0.1
}
